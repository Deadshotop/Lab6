...
import src.exceptions.InvalidArgument;
...
import static src.product.Condition.*;
/**
...
public class FileManager extends IOManager {
    ...
    public FileManager(HashTableManager products, String path, boolean script, NavigableSet<File> callHistory, StringManager stringManager, Execute execute) throws FileNotFoundException, RecursiveScript, NullPointerException {
        super(products);
    ...
    public void readData() {
        if (script) {
            while (lineNum < dataFile.length - 1) {
                exit = execute.doCommand(dataFile[lineNum++]);
                if (exit) {
                    break;
                }
            }
